			...
			<c:when test="${not empty userList}">
				...
				<tr>
					<td class='center' style="width: 30px;">
						<c:if test="${user.userName != 'admin'}"><label><input type='checkbox' name='ids' value="${user.userId }"/><span class="lbl"></span></label></c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>user/doDeleteUser?userId="+userId+"&tm="+new Date().getTime();
			...
			bootbox.confirm("确定要删除选中的数据吗?", function(result) {
				if(result) {
					...
					if(str==''){
						...
						$.ajax({
							type: "POST",
							url: '<%=basePath%>user/doDeleteAllUser?tm='+new Date().getTime(),
					    	data: {userIds:str},
							dataType:'json',
