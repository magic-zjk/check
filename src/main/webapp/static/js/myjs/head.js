...
$(function(){if('main'== locat[3] || 'index' == locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
...
function siMenu(id,fid,MENU_NAME,MENU_URL){	
	...
	console.log("id="+id+",fid="+fid+",mid="+mid+",fmid="+fmid);
	top.mainFrame.tabAddHandler(id,MENU_NAME,MENU_URL);
	if(MENU_URL != "druid/index.html"){
		jzts();
	}
}
function setMenuTitleActive(id,fid){
	console.log("测试："+id+","+fid);
	alert("测试："+id+","+fid);
	if(id != mid){
		$("#"+mid).removeClass();
		mid = id;
...
function editUserH(userid){
	 ...
	 diag.Drag=true;
	 diag.Title ="个人资料";
	 diag.URL = locat+'/user/toEditUserSelf.do?userId='+userid;
	 diag.Width = 225;
	 diag.Height = 389;
